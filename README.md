...
1.  Copy the content of `widget.js` and paste into an JSX note in Trilium. Under this note,
2.  Create a child code note (select `JS frontend`, paste the content of `utils.js`.
3.  Import `antidote.js` under `widget.js`
4.  Create a CSS note and paste the content of `widget.css` into it.

After reload the front-end, you will be able to see a button at the bottom right for text notes.

## Limitation

This extension uses the local Connectix rather than Antidote Web. To connect to Connectix, we need to know the port first. However, the port is not fixed once Connectix has started. We need to manually tell the extension the port.
